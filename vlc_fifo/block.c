 ...
 * Copyright (C) 2007-2009 R茅mi Denis-Courmont
 *
...
#else//干脆给他改进一下， 自动适应系统
#define BLOCK_ALIGN       sizeof(void*)
#define BLOCK_PADDING  BLOCK_ALIGN
#endif
block_t *block_Alloc (size_t size)
{
    if (unlikely(size >> 27))
    {//to limit the size <= 2^26
        errno = ENOBUFS;
        return NULL;
    }
    ...
    //位运算符只能为 整型 和字符型 ，这里没办法自动适应
    //b->p_buffer = (void *)( (void*)(b->p_buffer) & (void*)(~(BLOCK_ALIGN - 1)));
    b->i_buffer = size;
